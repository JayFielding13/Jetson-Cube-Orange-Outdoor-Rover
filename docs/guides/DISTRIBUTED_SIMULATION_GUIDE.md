...
**Architecture:**
- **Desktop (100.73.129.15)**: Runs Gazebo simulation, RViz (GPU-accelerated)
- **Jetson (100.91.191.47)**: Develops control code, monitors topics, sends commands
...
```bash
ssh jay@100.73.129.15 "pkill gzserver; pkill gzclient"
```

**Check for X11 forwarding (if using SSH with GUI):**
```bash
ssh -X -i ~/.ssh/jetson_to_desktop jay@100.73.129.15
