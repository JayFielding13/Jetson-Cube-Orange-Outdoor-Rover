...
class SSHDashboard:
    def __init__(self, root):
        self.root = root
        self.root.title("Raspberry Pi SSH Dashboard")
    ...
    def setup_gui(self):
        ...
        # Upload button
        self.upload_btn = ttk.Button(file_frame, text="Upload Python File", 
                                   command=self.upload_file, state="disabled")
        self.upload_btn.grid(row=0, column=0, sticky=(tk.W, tk.E), pady=(0, 5))
        
        # Remote path entry
        ttk.Label(file_frame, text="Remote Path:").grid(row=1, column=0, sticky=tk.W)
        ...
        self.remote_path_entry.grid(row=2, column=0, sticky=(tk.W, tk.E), pady=(0, 5))
        
        # Quick run button
        self.run_btn = ttk.Button(file_frame, text="Run Last Uploaded File", 
                                command=self.run_python_file, state="disabled")
        self.run_btn.grid(row=3, column=0, sticky=(tk.W, tk.E))
        ...
        self.activate_btn.grid(row=4, column=0, sticky=(tk.W, tk.E), pady=(5, 0))
        
        # Test X11 button
        self.test_x11_btn = ttk.Button(file_frame, text="Test X11 Display", 
                                     command=self.test_x11_display, state="disabled")
        self.test_x11_btn.grid(row=5, column=0, sticky=(tk.W, tk.E), pady=(5, 0))
    ...
    def on_connection_success(self):
        ...
        self.terminal_output.insert(tk.END, "Connected to Raspberry Pi\n")
    ...
    def execute_command_internal(self, command):
        ...
        def execute_thread():
            try:
                ...
                if self.x11_var.get():
                    # Use the DISPLAY variable from .bashrc and ensure proper setup
                    x11_setup = "source ~/.bashrc && export DISPLAY=192.168.254.14:0"
    ...
    def save_terminal_logs(self):
        ...
        try:
            ...
            if file_path:
                ...
                header = f"SSH Dashboard Terminal Log\n"
