...
Author: Claude Code Assistant
    ...
    def execute_command(self, pi_type):
        ...
        # Get command from appropriate input field
        command_var = getattr(self, f"{pi_type}_command_var")
        terminal_output = getattr(self, f"{pi_type}_terminal_output")
        ...
        def execute_thread():
            try:
                ...
                # Execute with PTY allocation for real-time output
                stdin, stdout, stderr = pi_connection.ssh.exec_command(command, get_pty=True)
                
                # Store channel for potential stopping
                setattr(self, f"{pi_type}_current_channel", stdout.channel)
    ...
    def stop_remote_program(self, pi_type):
        ...
        # Try to interrupt current channel
        current_channel = getattr(self, f"{pi_type}_current_channel", None)
        if current_channel:
            try:
                current_channel.send('\x03')  # Send Ctrl+C
                self.root.after(0, lambda: terminal_output.insert(tk.END, "\n[Program interrupted]\n"))
                self.root.after(0, lambda: terminal_output.see(tk.END))
            except:
                pass
        
        # Also send pkill as backup
        def stop_thread():
            try:
                stdin, stdout, stderr = pi_connection.ssh.exec_command("pkill -f python")
                self.root.after(0, lambda: terminal_output.insert(tk.END, "[All python processes stopped]\n"))
                self.root.after(0, lambda: terminal_output.see(tk.END))
            except Exception as e:
