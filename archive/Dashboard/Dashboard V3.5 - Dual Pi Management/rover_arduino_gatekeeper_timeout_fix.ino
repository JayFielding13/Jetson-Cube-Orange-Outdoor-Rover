...
#define EMERGENCY_DISTANCE 15.0  // Emergency stop at 15cm (increased from 10cm)
#define ULTRASONIC_TIMEOUT 30000  // 30ms timeout (increased from 5ms) - SENSOR FIX
...
// Sensor diagnostics (NEW)
unsigned long sensor_timeout_count = 0;
unsigned long sensor_success_count = 0;
float last_valid_distance = -1;
...
void setup() {
  ...
  Serial.println("Arduino Gatekeeper v1.3 - SENSOR TIMEOUT FIX");
  ...
  Serial.println("🔧 SENSOR FIX: Timeout increased from 5ms to 30ms");
...
void applyMotorCommands() {
  ...
  switch (current_mode) {
    case MODE_AUTONOMOUS:
      // AUTONOMOUS MODE: Full safety systems active
      if (emergency_stop) {
        // Emergency stop applies in autonomous mode
        setMotorSpeeds(0, 0);
        return;
      }
...
void checkEmergencyStop() {
  // Quick emergency stop check with enhanced diagnostics
  ...
  if (distance > 0) {
    ...
    // Don't trigger emergency on sensor timeout - use last known distance
    if (last_valid_distance > 0 && last_valid_distance < EMERGENCY_DISTANCE) {
      emergency_stop = true;
    }
  }
}
...
float readUltrasonicDistance() {
  // Enhanced ultrasonic distance measurement with diagnostics
  ...
  // Read echo with EXTENDED TIMEOUT - SENSOR FIX
  unsigned long duration = pulseIn(FRONT_ECHO_PIN, HIGH, ULTRASONIC_TIMEOUT);
  
  if (duration == 0) {
    return -1; // Timeout - but now with 30ms instead of 5ms
  }
...
void sendDataToPi() {
  // Send status data to Pi including ultrasonic distance with diagnostics
  int ch1_value = map(constrain(ch1_pulse_width, RC_MIN_PULSE, RC_MAX_PULSE), 
                     RC_MIN_PULSE, RC_MAX_PULSE, -1000, 1000);
  int ch2_value = map(constrain(ch2_pulse_width, RC_MIN_PULSE, RC_MAX_PULSE), 
                     RC_MIN_PULSE, RC_MAX_PULSE, -1000, 1000);
  ...
  // NEW: Add sensor diagnostics
  Serial.print(",\"sensor_timeouts\":");
  Serial.print(sensor_timeout_count);
  Serial.print(",\"sensor_success\":");
  Serial.print(sensor_success_count);
  if (last_valid_distance > 0) {
    Serial.print(",\"last_valid_distance\":");
    Serial.print(last_valid_distance, 1);
  }
