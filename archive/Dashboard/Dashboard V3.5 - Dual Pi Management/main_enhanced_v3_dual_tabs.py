...
class EnhancedSSHDashboard:
    ...
    def setup_system_monitoring_tab(self):
        """Setup system monitoring tab for both Pis"""
        ...
        nav_monitor_frame = ttk.LabelFrame(monitor_paned, text="📡 Navigation Pi System", padding="5")
        monitor_paned.add(nav_monitor_frame, weight=1)
        
        self.nav_monitor_text = scrolledtext.ScrolledText(nav_monitor_frame, height=30, width=60)
        self.nav_monitor_text.pack(fill='both', expand=True)
        
        # Companion Pi monitoring
        comp_monitor_frame = ttk.LabelFrame(monitor_paned, text="🎨 Companion Pi System", padding="5")
        monitor_paned.add(comp_monitor_frame, weight=1)
        
        self.comp_monitor_text = scrolledtext.ScrolledText(comp_monitor_frame, height=30, width=60)
        self.comp_monitor_text.pack(fill='both', expand=True)
        
        # Control buttons
        control_frame = ttk.Frame(monitoring_frame)
        control_frame.pack(fill='x', padx=5, pady=5)
        
        ttk.Button(control_frame, text="🔄 Refresh Nav Pi Status", 
                  command=self.refresh_nav_status).pack(side='left', padx=2)
        ttk.Button(control_frame, text="🔄 Refresh Comp Pi Status", 
                  command=self.refresh_comp_status).pack(side='left', padx=2)
