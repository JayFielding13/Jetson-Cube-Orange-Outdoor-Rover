#!/usr/bin/env python3
"""
Raspberry Pi SSH Dashboard V3.6 - Dual Pi Dedicated Tabs Edition
================================================================

Enhanced SSH dashboard with separate dedicated tabs for each Pi:
...
Key Improvements:
- Separate file management for each Pi with independent connections
- Clear Pi identification in tabs and interfaces
- Individual upload/download operations per Pi
- Better organization for dual Pi rover development

Author: Claude Code Assistant
Version: 3.6 - Dual Pi Dedicated Tabs Edition
"""
...
class EnhancedSSHDashboard:
    """Enhanced SSH Dashboard with dual Pi dedicated tabs"""
    
    def __init__(self, root):
        self.root = root
        self.root.title("Rover SSH Dashboard V3.6 - Dual Pi Dedicated Tabs")
    ...
    def setup_system_monitoring_tab(self):
        """Setup terminal execution tab for both Pis"""
        monitoring_frame = ttk.Frame(self.notebook)
        self.notebook.add(monitoring_frame, text="💻 Terminal")
        
        # Split into two terminal panels
        terminal_paned = ttk.PanedWindow(monitoring_frame, orient='horizontal')
        terminal_paned.pack(fill='both', expand=True, padx=5, pady=5)
        
        # Navigation Pi terminal
        nav_terminal_frame = ttk.LabelFrame(terminal_paned, text="📡 Navigation Pi Terminal", padding="5")
        terminal_paned.add(nav_terminal_frame, weight=1)
        
        # Nav Pi command input
        nav_cmd_frame = ttk.Frame(nav_terminal_frame)
        nav_cmd_frame.pack(fill='x', padx=2, pady=2)
        
        self.nav_command_var = tk.StringVar()
        ttk.Label(nav_cmd_frame, text="$").pack(side='left')
        nav_cmd_entry = ttk.Entry(nav_cmd_frame, textvariable=self.nav_command_var)
        nav_cmd_entry.pack(side='left', fill='x', expand=True, padx=2)
        nav_cmd_entry.bind('<Return>', lambda e: self.execute_command("nav"))
        
        # Nav Pi terminal buttons
        nav_btn_frame = ttk.Frame(nav_terminal_frame)
        nav_btn_frame.pack(fill='x', padx=2, pady=2)
        
        ttk.Button(nav_btn_frame, text="▶️ Execute", 
                  command=lambda: self.execute_command("nav")).pack(side='left', padx=2)
        ttk.Button(nav_btn_frame, text="⏹️ Stop", 
                  command=lambda: self.stop_remote_program("nav")).pack(side='left', padx=2)
        ttk.Button(nav_btn_frame, text="🧹 Clear", 
                  command=lambda: self.clear_terminal("nav")).pack(side='left', padx=2)
        
        # Nav Pi terminal output
        self.nav_terminal_output = scrolledtext.ScrolledText(nav_terminal_frame, height=25, width=60)
        self.nav_terminal_output.pack(fill='both', expand=True, padx=2, pady=2)
        
        # Companion Pi terminal
        comp_terminal_frame = ttk.LabelFrame(terminal_paned, text="🎨 Companion Pi Terminal", padding="5")
        terminal_paned.add(comp_terminal_frame, weight=1)
        
        # Comp Pi command input
        comp_cmd_frame = ttk.Frame(comp_terminal_frame)
        comp_cmd_frame.pack(fill='x', padx=2, pady=2)
        
        self.comp_command_var = tk.StringVar()
        ttk.Label(comp_cmd_frame, text="$").pack(side='left')
        comp_cmd_entry = ttk.Entry(comp_cmd_frame, textvariable=self.comp_command_var)
        comp_cmd_entry.pack(side='left', fill='x', expand=True, padx=2)
        comp_cmd_entry.bind('<Return>', lambda e: self.execute_command("comp"))
        
        # Comp Pi terminal buttons
        comp_btn_frame = ttk.Frame(comp_terminal_frame)
        comp_btn_frame.pack(fill='x', padx=2, pady=2)
        
        ttk.Button(comp_btn_frame, text="▶️ Execute", 
                  command=lambda: self.execute_command("comp")).pack(side='left', padx=2)
        ttk.Button(comp_btn_frame, text="⏹️ Stop", 
                  command=lambda: self.stop_remote_program("comp")).pack(side='left', padx=2)
        ttk.Button(comp_btn_frame, text="🧹 Clear", 
...
Perfect for dual Pi rover development and deployment!
...
Author: Claude Code Assistant
    ...
    def execute_command(self, pi_type):
        ...
        # Get command and terminal widgets
        command_var = self.nav_command_var if pi_type == "nav" else self.comp_command_var
        terminal_output = self.nav_terminal_output if pi_type == "nav" else self.comp_terminal_output
        ...
        def execute_thread():
            try:
                ...
                self.root.after(0, lambda: terminal_output.insert(tk.END, f"$ {command}\n"))
                
                # Execute with PTY for real-time output
                ...
                while not stdout.channel.exit_status_ready():
                    if stdout.channel.recv_ready():
                        ...
                        if data:
                            self.root.after(0, lambda d=data: terminal_output.insert(tk.END, d))
                            self.root.after(0, lambda: terminal_output.see(tk.END))
                    
                    if stderr.channel.recv_stderr_ready():
                        error_data = stderr.channel.recv_stderr(1024).decode('utf-8', errors='replace')
                        if error_data:
                            self.root.after(0, lambda d=error_data: terminal_output.insert(tk.END, f"[ERROR] {d}"))
                            self.root.after(0, lambda: terminal_output.see(tk.END))
                ...
                if remaining_out:
                    self.root.after(0, lambda d=remaining_out: terminal_output.insert(tk.END, d))
                
                remaining_err = stderr.read().decode('utf-8', errors='replace')
                if remaining_err:
                    self.root.after(0, lambda d=remaining_err: terminal_output.insert(tk.END, f"[ERROR] {d}"))
                    
                self.root.after(0, lambda: terminal_output.see(tk.END))
                
            except Exception as e:
                error_msg = str(e)
                self.root.after(0, lambda: terminal_output.insert(tk.END, f"Command failed: {error_msg}\n"))
                self.root.after(0, lambda: terminal_output.see(tk.END))
        
        threading.Thread(target=execute_thread, daemon=True).start()
    
    def stop_remote_program(self, pi_type):
        """Stop running programs on specified Pi"""
        pi_connection = self.nav_pi if pi_type == "nav" else self.comp_pi
        pi_name = "Navigation Pi" if pi_type == "nav" else "Companion Pi"
        terminal_output = self.nav_terminal_output if pi_type == "nav" else self.comp_terminal_output
        ...
        def stop_thread():
            try:
                stdin, stdout, stderr = pi_connection.ssh.exec_command("pkill -f python")
                self.root.after(0, lambda: terminal_output.insert(tk.END, "[Python processes stopped]\n"))
                self.root.after(0, lambda: terminal_output.see(tk.END))
            except Exception as e:
                self.root.after(0, lambda: terminal_output.insert(tk.END, f"[Stop failed: {e}]\n"))
                self.root.after(0, lambda: terminal_output.see(tk.END))
    ...
    def refresh_nav_status(self):
        """Refresh Navigation Pi system status"""
        if not self.nav_pi.is_connected():
            messagebox.showwarning("Warning", "Navigation Pi is not connected")
            return
        ...
        def status_thread():
            try:
                ...
                commands = [
                    ...
                    ("Disk Usage", "df -h"),
                    ("Memory Usage", "free -h"),
                    ("CPU Info", "top -bn1 | head -n 20"),
                    ("Network Info", "ip addr show"),
                    ("Running Processes", "ps aux | head -n 20")
                ]
                
                status_text = f"=== Navigation Pi System Status - {datetime.now().strftime('%H:%M:%S')} ===\n\n"
                ...
                self.nav_monitor_text.insert(1.0, status_text)
                
            except Exception as e:
                messagebox.showerror("Error", f"Failed to get Navigation Pi status: {e}")
        
        threading.Thread(target=status_thread, daemon=True).start()
    
    def refresh_comp_status(self):
        """Refresh Companion Pi system status"""
        if not self.comp_pi.is_connected():
            messagebox.showwarning("Warning", "Companion Pi is not connected")
            return
        ...
        def status_thread():
            try:
                ...
                status_text = f"=== Companion Pi System Status - {datetime.now().strftime('%H:%M:%S')} ===\n\n"
                ...
                self.comp_monitor_text.insert(1.0, status_text)
                
            except Exception as e:
                messagebox.showerror("Error", f"Failed to get Companion Pi status: {e}")
    ...
    def show_system_info(self):
        ...
        info = f"""
Rover SSH Dashboard V3.6 - Dual Pi Dedicated Tabs Edition
...
System Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}
    ...
    def show_about(self):
        """Show about dialog"""
        about_text = """
Rover SSH Dashboard V3.6
Dual Pi Dedicated Tabs Edition
