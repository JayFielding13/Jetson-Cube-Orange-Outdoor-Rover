#!/usr/bin/env python3
"""
Autonomous Bluetooth-Hybrid Navigator v4
WORKING: Fixed threading integration for callback-based RSSI scanning
...
class BluetoothHybridNavigatorV4:
    """
    WORKING autonomous rover navigator with properly integrated callback-based RSSI scanning
    ...
    def __init__(self, arduino_port='/dev/ttyUSB0', baud=115200):
        ...
        # Fixed Bluetooth state with proper threading
        ...
        print("🚀 Bluetooth-Hybrid Navigator v4 (WORKING RSSI) Initialized")
        ...
        print(f"📡 BT Method: Proper thread integration with callback scanning")
    ...
    def bluetooth_detection_callback(self, device, advertisement_data):
        """Thread-safe callback function for real-time BLE detection with real RSSI"""
        if device.address.upper() == self.bluetooth_state['target_mac'].upper():
            ...
            with self.detection_lock:
                ...
                # Update state immediately in callback
                ...
                if self.bluetooth_state['detection_count'] % 5 == 0:
                    ...
                    print(f"📡 BlueCharm WORKING #{self.bluetooth_state['detection_count']}:")
    ...
    def start(self):
        """Start the hybrid navigator with proper thread integration"""
        ...
        # Start WORKING Bluetooth with proper thread integration
        if self.bluetooth_state['enabled']:
            ...
            print("📡 WORKING callback-based Bluetooth scanner started")
        
        self.arduino_thread.start()
        self.navigation_thread.start()
        
        print("🚀 WORKING Navigator started")
        print("🎯 Ready for autonomous navigation with WORKING Bluetooth RSSI tracking")
        return True
    
    def bluetooth_thread_worker(self):
        """Dedicated thread worker for Bluetooth scanning with proper asyncio integration"""
        print("📡 Starting WORKING Bluetooth thread worker...")
        ...
        try:
            ...
            print(f"❌ Bluetooth thread worker error: {e}")
        finally:
            try:
                loop.close()
            except:
    ...
    async def bluetooth_scanner_main(self):
        ...
        print("📡 WORKING Bluetooth scanner main started")
        ...
        try:
            while self.running:
                ...
                if self.robot_state.get('mode', 0) == 2:
                    if not self.bluetooth_state['scanner_running']:
                        # Start scanner
                        print("📡 Starting WORKING BLE callback scanner...")
                        scanner = BleakScanner(self.bluetooth_detection_callback)
                        await scanner.start()
                        self.bluetooth_state['scanner_running'] = True
                        print("📡 WORKING BLE scanner started successfully")
                    
                    # Check for detection timeout while running
                    current_time = time.time()
                    ...
                    if self.bluetooth_state['scanner_running'] and scanner:
                        print("📡 Stopping WORKING BLE scanner...")
                        await scanner.stop()
            ...
            print(f"❌ Bluetooth scanner main error: {e}")
        
        finally:
            # Cleanup scanner
            if scanner and self.bluetooth_state['scanner_running']:
                try:
                    print("📡 Cleaning up WORKING BLE scanner...")
                    await scanner.stop()
                    self.bluetooth_state['scanner_running'] = False
                except Exception as e:
    ...
    def log_enhanced_status(self, ultrasonic_distance):
        """Enhanced status logging with WORKING Bluetooth RSSI info"""
        ...
        # Add WORKING Bluetooth info if available (thread-safe)
        if self.bluetooth_state['enabled']:
            with self.detection_lock:
                if self.bluetooth_state['target_distance']:
                    ...
                    status_line += f" | BT: {bt_distance:.1f}m ({bt_rssi}dBm WORK)"
                else:
    ...
    def print_enhanced_statistics(self):
        """Print enhanced navigation and WORKING Bluetooth statistics"""
        runtime = time.time() - self.stats['start_time']
        ...
        # Enhanced WORKING Bluetooth statistics
        if self.bluetooth_state['enabled']:
            with self.detection_lock:
                print("📡 === WORKING RSSI Bluetooth Statistics ===")
    ...
    def stop(self):
        ...
        print("🛑 Stopping WORKING Navigator...")
        self.running = False
        ...
        print("👋 WORKING Navigator shutdown complete")

def main():
    """Main entry point for the WORKING hybrid navigator"""
    print("=" * 75)
    print("🤖 AUTONOMOUS BLUETOOTH-HYBRID NAVIGATOR v4 (WORKING RSSI)")
    print("=" * 75)
    print()
    print("Features:")
    ...
    print("  📡 WORKING: Properly integrated callback-based RSSI scanning")
    ...
    print("  🧵 Fixed threading integration - no more task warnings")
    print()
    ...
    navigator = BluetoothHybridNavigatorV4()
    
    try:
        if navigator.start():
            print("✅ WORKING Navigator ready - switch to autonomous mode")
            print("📡 WORKING RSSI Bluetooth tracking will activate automatically")
            print("🔍 Watch for WORKING signal strength values!")
