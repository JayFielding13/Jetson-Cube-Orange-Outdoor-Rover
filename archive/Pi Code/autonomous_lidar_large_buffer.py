#!/usr/bin/env python3
"""
Large Buffer LiDAR Navigator
Attempts to override buffer size limit for complete scans
"""
...
class LargeBufferNavigator:
    def __init__(self, arduino_port='/dev/ttyUSB0', lidar_port='/dev/ttyUSB1', baud=115200):
        ...
        print("🚀 Large Buffer Navigator Initialized")
        print("🎯 Attempting to use larger buffer for complete scans")
    ...
    def connect_lidar(self):
        """Connect to LiDAR sensor with buffer modifications"""
        ...
        try:
            ...
            # Try to modify buffer size if possible
            try:
                # Check if we can access internal buffer settings
                if hasattr(self.lidar, '_buffer_size'):
                    print(f"📊 Current buffer size: {self.lidar._buffer_size}")
                    self.lidar._buffer_size = 8000  # Try 8KB
                    print(f"📊 Buffer size increased to: {self.lidar._buffer_size}")
                elif hasattr(self.lidar, 'SCAN_BYTE'):
                    print(f"📊 Found SCAN_BYTE constant: {self.lidar.SCAN_BYTE}")
                
                # Check serial buffer
                if hasattr(self.lidar, '_serial'):
                    print(f"📊 Serial port: {self.lidar._serial}")
            except Exception as e:
                print(f"📊 Buffer modification not possible: {e}")
    ...
    def start(self):
        ...
        print("🚀 Large Buffer Navigator started")
        return True
    ...
    def lidar_loop(self):
        """LiDAR processing with multiple buffer size attempts"""
        consecutive_failures = 0
        max_failures = 3
        
        while self.running:
            try:
                if LIDAR_AVAILABLE and self.lidar and consecutive_failures < max_failures:
                    success = self.try_multiple_scan_methods()
                    if success:
                        consecutive_failures = 0
                    else:
                        consecutive_failures += 1
                        if consecutive_failures >= max_failures:
                            print(f"⚠️ {max_failures} consecutive LiDAR failures - using simulation")
                else:
            ...
            time.sleep(0.5)  # 2Hz LiDAR processing
    
    def try_multiple_scan_methods(self):
        """Try different scan methods to avoid buffer overflow"""
        methods = [
            self.try_basic_scan,
            self.try_small_buffer_scan,
            self.try_express_scan,
            self.try_force_scan
        ]
    ...
    def try_basic_scan(self):
        """Try basic iter_scans()"""
        scan_generator = self.lidar.iter_scans()
        scan = next(scan_generator)
        return self.process_scan_to_quadrants(scan)
    
    def try_small_buffer_scan(self):
        """Try with small buffer"""
        scan_generator = self.lidar.iter_scans(max_buf_meas=100)
        scan = next(scan_generator)
        return self.process_scan_to_quadrants(scan)
    
    def try_express_scan(self):
        """Try express scan if available"""
        try:
            scan_generator = self.lidar.iter_scans(scan_type='express')
            scan = next(scan_generator)
            return self.process_scan_to_quadrants(scan)
        except TypeError:
            # scan_type not supported
            return False
    
    def try_force_scan(self):
        """Try forcing scan start"""
        try:
            if hasattr(self.lidar, 'start_scan'):
    ...
    def process_scan_to_quadrants(self, scan):
        """Process scan data into 4 quadrants"""
    ...
    def navigation_loop(self):
        ...
        while self.running:
            try:
                ...
                # Compute navigation
                left_speed, right_speed = self.compute_navigation(front_obstacle)
                ...
                if time.time() % 3 < 0.3:
                    ...
                    print(f"🧭 Scan #{scan_num} | Front: {front_obstacle:.1f}cm | {sensor_mode}: {points}pts")
            ...
            time.sleep(0.5)  # 2Hz navigation for smooth movement
    
    def compute_navigation(self, front_obstacle):
        ...
        if self.nav_state == 'STRAIGHT':
            if front_obstacle < self.danger_threshold:
                ...
                self.turn_direction = 'left'  # Default turn direction
                print(f"🚨 DANGER! Front={front_obstacle:.1f}cm - turning left")
    ...
    def stop(self):
        ...
        print("🛑 Large Buffer Navigator stopped")

def main():
    print("=" * 60)
    print("🚀 LARGE BUFFER LIDAR NAVIGATOR")
    print("=" * 60)
    print()
    print("Strategy: Multiple scan methods to overcome buffer limitations")
    print("Features: Fallback methods, slower rates, better error handling")
    print()
    
    navigator = LargeBufferNavigator()
    
    try:
        if navigator.start():
            print("✅ Large Buffer Navigator ready")
