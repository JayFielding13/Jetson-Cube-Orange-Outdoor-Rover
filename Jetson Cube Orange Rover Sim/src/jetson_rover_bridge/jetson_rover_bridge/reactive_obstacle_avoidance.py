...
class ReactiveObstacleAvoidance(Node):
    def __init__(self):
        ...
        self.create_subscription(Range, '/ultrasonic/front', self._ultrasonic_front, 10)
        self.create_subscription(Range, '/ultrasonic/corner_left', self._ultrasonic_corner_left, 10)
        self.create_subscription(Range, '/ultrasonic/side_left', self._ultrasonic_side_left, 10)
        self.create_subscription(Range, '/ultrasonic/rear', self._ultrasonic_rear, 10)
        self.create_subscription(Range, '/ultrasonic/side_right', self._ultrasonic_side_right, 10)
        self.create_subscription(Range, '/ultrasonic/corner_right', self._ultrasonic_corner_right, 10)
    ...
    # Individual callbacks for each ultrasonic sensor (avoids lambda/closure issues)
    # Each callback stores data directly to avoid any closure/argument passing issues
    def _ultrasonic_front(self, msg):
        self.ultrasonic_readings['front'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')

    def _ultrasonic_corner_left(self, msg):
        self.ultrasonic_readings['corner_left'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')

    def _ultrasonic_side_left(self, msg):
        self.ultrasonic_readings['side_left'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')

    def _ultrasonic_rear(self, msg):
        self.ultrasonic_readings['rear'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')

    def _ultrasonic_side_right(self, msg):
        self.ultrasonic_readings['side_right'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')

    def _ultrasonic_corner_right(self, msg):
        self.ultrasonic_readings['corner_right'] = msg.range if 0.1 <= msg.range <= 6.0 else float('inf')
    ...
    def process_avoidance(self):
        ...
        critical = bool(min_dist < self.critical_dist)
